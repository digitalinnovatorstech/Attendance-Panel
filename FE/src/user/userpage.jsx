...
const AttendanceCard = () => {
  ...
  const punchIn = async (reason = "") => {
    try {
      ...
      setPunchInTime(now); // Set punch in time
      setIsPunchedIn(true); // Set punched in status last to trigger timer
  ...
  // Timer effect that runs when punched in
  useEffect(() => {
    if (isPunchedIn && !punchOutTime) {
      // Calculate initial elapsed seconds if punch in time exists
      if (punchInTime) {
        const now = new Date();
        const punchInDate = new Date(punchInTime);
        const initialElapsed = Math.floor((now - punchInDate) / 1000);
  ...
  }, [isPunchedIn, punchOutTime, punchInTime]);

  // Update the button styles to show red for punch out
  const getPunchButtonStyles = () => ({
    backgroundColor: isPunchedIn ? "#dc2626" : "#1b5e20",
    color: "white",
    "&:hover": {
      backgroundColor: isPunchedIn ? "#b91c1c" : "#15803d",
    },
    fontWeight: 600,
      ...
      >
        ...
        <DialogContent dividers>
          <TextField
            label="To"
            fullWidth
            value="maheswar.reddy@innovatorstech.com"
            sx={{ mb: 2 }}
